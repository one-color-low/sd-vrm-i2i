...
const VrmViewer = forwardRef((props: VRMViewerProps, ref) => {
  ...
  function captureCanvas() {
    if (canvasRef.current) {
      console.log("canvasRef exist");
      console.log(canvasRef);
      return canvasRef.current.toDataURL();
    }
  }

  const canvasInitializer = useCallback(
    async () => {
  ...
  return (
    <div className="flex flex-col">
      <div className={'sm:w-[390px] w-[60vw] sm:h-[480px] h-[583px]'}>
        ...
        <canvas id="VrmViewerCanvas" ref={canvasRef} className={'h-full w-full rounded-24 bg-[--charcoal-surface3]'}></canvas>
